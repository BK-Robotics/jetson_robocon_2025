...
class PlaneDetector:
    ...
    def analyze_plane_geometry(self, plane_points):
        ...
        # Góc với các trục
        angle_x = np.degrees(np.arccos(np.abs(np.dot(normal, [1, 0, 0]))))
        angle_y = np.degrees(np.arccos(np.abs(np.dot(normal, [0, 1, 0]))))
        angle_z = np.degrees(np.arccos(np.abs(np.dot(normal, [0, 0, 1]))))
